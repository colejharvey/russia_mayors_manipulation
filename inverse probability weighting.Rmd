...
w <- ipwtm(
  ...
  denominator = ~ treated.mod.1lag + treated.mod.2lag + urgov + lnAvgSalary + comp.scale2 + known.opp.mayor +  margin.most.recent +  dem  + years.with.elections + urvoteshare.1lag + turncoef.1lag + n.precincts,
  id = city_id,
  timevar=year.x,
  type="first",
  data = data_long_sort)
...
sjPlot::tab_model(gee.iptw, gee.iptw2, show.ci = F, p.style = "stars", digits = 3, show.se = T, collapse.se =  T) #This is the model we want in the paper
...
w <- ipwtm(
  ...
  denominator = ~ treated.mod.1lag + treated.mod.2lag + treated.mod.3lag +  urgov + lnAvgSalary + comp.scale2 + known.opp.mayor +  margin.most.recent + margin.most.recent + dem  + years.with.elections + n.precincts + appointed.share,
  id = city_id,
  timevar=year.x,
  type="first",
  data = data_long_sort)
...
###########################################################################################
gee.bias <- geeglm(turnout.coef~ treated.post.treatment +  presidential + xconst.lag +
                     putin.app.3mo + years.under.regime + n.precincts + cluster(city_id),
                    id=city_id,
                    data=data2, 
                    family=gaussian("identity"), 
                    corstr="ar1")
...
gee.iptw <- geeglm(turnout.coef~ treated.mod + treated.mod.1lag + treated.mod.2lag + treated.mod.3lag + presidential + xconst.lag +
                     putin.app.3mo + mayor.tenure + n.precincts + cluster(city_id),
                    id=city_id,
                    data=data2, 
                    family=gaussian("identity"), 
                    corstr="ar1", 
...
gee.iptw2 <- geeglm(ur.voteshare~ treated.mod + treated.mod.1lag + treated.mod.2lag + treated.mod.3lag + presidential + xconst.lag +
                     putin.app.3mo + mayor.tenure + turnout.coef + n.precincts + cluster(city_id),
                    id=city_id,
                    data=data2, 
                    family=gaussian("identity"), 
                    corstr="ar1", 
